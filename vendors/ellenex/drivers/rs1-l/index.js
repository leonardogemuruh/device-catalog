...
function decodeUplink(input) {
    ...
    result.serial = bytesToHex(input.bytes.slice(0, 2));

    const payloadType = input.bytes[2];

    let dataLength = payloadType === 0 ? 4 : 2;

    if(payloadType === 0) {
        result.pressure_output = readHex2bytes(input.bytes[3], input.bytes[4]);
        result.temperature_output = readHex2bytes(input.bytes[3], input.bytes[4]);

        if(input.thing.setup.sensorRange) {
            let sR = parseInt(input.thing.setup.sensorRange);

            let pressureValue = sR * (result.pressure_output - 4000) / (16000);
            ...
            let temperatureValue = sR * (result.temperature_output - 4000) / (16000);
        ...
        result.downlink_command = bytesToHex(input.bytes.slice(3, 4));
    }

    result.battery = (input.bytes[3+dataLength] * 0.1).toFixed(1);
